...
Places_Data<-read.xlsx2("Estimates_for_All_Counties.xlsx",  sheetIndex=1, startRow=5, colIndex=1:4, stringsAsFactors=FALSE, header=TRUE)
...
write.csv(Places_Data, file = "Household_Income.csv", row.names = FALSE)
data<- read.csv("Household_Income.csv", header=TRUE, stringsAsFactors=FALSE)

### Second method, from the csv file ###
Places_Data<- read.csv("Estimates_Counties.csv", header=TRUE, stringsAsFactors=FALSE)
